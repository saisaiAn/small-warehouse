...
create table Emp (
  ...
  empType int not null #判断用户状态，是否登录
);
##部门表
...
price  DOUBLE not null,#商品价格(price)	Double	10	否		
...
commodityType int not null#判断商品状态，上架/下架
);
##商品分类表
...
CREATE TABLE shoppingCar(
	carNo int auto_increment primary key,#购物车编号(?carNo)	int	30	否	主键	自增
  shoppingEmpNo int not null,#购物车用户(shoppingEmpNo)	int	not null	否		外键
  shoppingCommodityNo int not null,#商品编号(shoppingCommodityNo)	 int not null	否	
  commoditySum int not null#商品数量(commoditySum)	int not null		否		
)
